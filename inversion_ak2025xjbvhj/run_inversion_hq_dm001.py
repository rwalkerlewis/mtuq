#!/usr/bin/env python
"""
High-quality station inversion with consistent dM = 0.01 throughout
"""
...
# Magnitude search parameters
dM = 0.01  # Magnitude increment
mag_min = 5.80
mag_max = 6.30
...
print('='*70)
print(f'HIGH-QUALITY STATION INVERSION (dM = {dM})')
print('='*70)
...
# Create magnitude array with consistent dM spacing
magnitudes = np.arange(mag_min, mag_max + dM/2, dM)
magnitudes = np.round(magnitudes, 2)  # Avoid floating point issues

print(f'\nSearching {len(magnitudes)} magnitudes from Mw {magnitudes[0]:.2f} to {magnitudes[-1]:.2f} (dM = {dM})')

# Search each magnitude
mag_results = []

for i, test_mag in enumerate(magnitudes):
    ...
    # Progress update every 10 magnitudes
    if (i + 1) % 10 == 0 or i == 0 or i == len(magnitudes) - 1:
        print(f'  [{i+1}/{len(magnitudes)}] Mw {test_mag:.2f}: misfit = {min_misfit:.6f}')
...
# Output
output_dir = os.path.join(script_dir, 'results_hq_dm001')
...
try:
    plot_data_greens2(
        os.path.join(output_dir, f'{event_id}_FMT_hq_dm001_waveforms.png'),
...
try:
    plot_beachball(
        os.path.join(output_dir, f'{event_id}_FMT_hq_dm001_beachball.png'),
...
try:
    plot_misfit_lune(
        os.path.join(output_dir, f'{event_id}_FMT_hq_dm001_misfit_lune.png'),
        best_result['results']
    )
...
save_json(
    os.path.join(output_dir, f'{event_id}_FMT_hq_dm001_solution.json'),
    merged_dict
)
...
with open(os.path.join(output_dir, 'magnitude_misfit.txt'), 'w') as f:
    f.write(f'# Magnitude vs Misfit (dM = {dM})\n')
    f.write('# Mw\tMisfit\tVR(%)\n')
    for r in mag_results:
        vr = (1.0 - r['misfit']) * 100
        f.write(f"{r['magnitude']:.2f}\t{r['misfit']:.6f}\t{vr:.1f}\n")

print(f'  Saved magnitude-misfit curve')

# Save summary
with open(os.path.join(output_dir, 'summary.txt'), 'w') as f:
    f.write(f'HIGH-QUALITY INVERSION RESULTS (dM = {dM})\n')
    f.write('='*50 + '\n\n')
    ...
    f.write(f'Magnitude resolution: dM = {dM}\n')
    f.write(f'Magnitude range: Mw {mag_min:.2f} to {mag_max:.2f}\n')
    f.write(f'Magnitudes tested: {len(magnitudes)}\n\n')
    f.write(f'Results:\n')
