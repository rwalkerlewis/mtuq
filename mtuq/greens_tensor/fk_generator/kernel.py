...
def layer_parameters(k: float, omega: complex, vp: float, vs: float, 
                     rho: float, qp: float, qs: float, d: float, mu: float) -> Dict:
    """
...
def compound_matrix(ra: complex, rb: complex, r: complex, r1: complex, 
                    kd: float, mu2: float) -> np.ndarray:
    """
...
def haskell_matrix(ra: complex, rb: complex, r: complex, r1: complex,
                   kd: float, mu2: float) -> np.ndarray:
    """
...
def e_vector(ra: complex, rb: complex, r: complex, r1: complex, mu2: float) -> np.ndarray:
    """
...
def initial_g(ra: complex, rb: complex, r: complex, r1: complex, mu2: float) -> np.ndarray:
    """
    Initialize g vector in the bottom halfspace.
    
    g contains (E^-1)|_(ij)^(12) for P-SV and first row of SH E^-1.
    ...
    # P-SV: coefficients for the bottom halfspace
    denom = ra * rb - r1 * r1
...
def separate_source(si: np.ndarray, stype: int, updn: int, 
                    ra: complex, rb: complex, r: complex, r1: complex, 
...
def compute_kernel(
    ...
    for j in range(mb - 1, -1, -1):  # j = mb-1, ..., 0 (0-indexed)
        ...
        params = layer_parameters(
            k, omega, 
            model_vp[layer_idx], model_vs[layer_idx],
            model_rho[layer_idx], model_qp[layer_idx], model_qs[layer_idx],
            model_d[layer_idx], model_mu[layer_idx]
        )
